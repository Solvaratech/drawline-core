...
export class PostgreSQLHandler {
	...
	async connect(): Promise<void> {
		try {
			...
			logger.log("PostgreSQLHandler", `Connecting to ${config.connectionString.replace(/:[^:@]*@/, ':****@')}`);
	...
	async getCollections(selectedSchemas?: string[]): Promise<string[]> {
		...
		logger.log("PostgreSQLHandler", `Connected to database: ${dbResult.rows[0].current_database}`);
		...
		const result = await this.client.query(query, params.length > 0 ? [params] : undefined);
		logger.log("PostgreSQLHandler", `getCollections found ${result.rows.length} tables${selectedSchemas ? ` in schemas: ${selectedSchemas.join(', ')}` : ''}`);
	...
	async inferSchema(collectionName: string, sampleSize: number = 100): Promise<Record<string, unknown>> {
		...
		const foreignKeysMap = new Map<string, { table: string; column: string; constraintName: string }>();
		const constraintColumnCount = new Map<string, number>();
		fkResult.rows.forEach(r => {
			constraintColumnCount.set(r.constraint_name, (constraintColumnCount.get(r.constraint_name) || 0) + 1);
		});
		fkResult.rows.forEach(r => {
			const fkTable = r.foreign_schema === 'public' ? r.foreign_table : `${r.foreign_schema}.${r.foreign_table}`;
		...
		result.rows.forEach(row => {
			...
			const defaultValue = row.column_default ? row.column_default.toLowerCase() : "";

			if (pgType === "integer" || pgType === "smallint") {
				type = "integer";
			} else if (pgType === "bigint") {
				type = "integer";
			} else if (pgType.includes("numeric") || pgType.includes("decimal") || pgType.includes("real") || pgType.includes("double") || pgType === "money") {
				type = "number";
			...
			const compositePrimaryKeyIndex = isCompositePrimaryKey && isPrimaryKey
				? primaryKeyPositions.get(row.column_name)
				: undefined;
			const compositeKeyGroup = fk && (constraintColumnCount.get(fk.constraintName) || 0) > 1
				? fk.constraintName
				: undefined;
	 ...
	 */
	async detectRelationships(collections: string[], selectedSchemas?: string[]): Promise<DatabaseRelationship[]> {
		if (!this.client) throw new Error("Not connected to PostgreSQL");

		// Extract schemas from collections if selectedSchemas not provided
		const schemasToQuery = selectedSchemas && selectedSchemas.length > 0
			? selectedSchemas
		...
		fkResult.rows.forEach(row => {
			...
			if (!collections.includes(childTable) || !collections.includes(parentTable)) return;
		...
		for (const col of allColumns) {
			...
			if (explicitKeys.has(fullKey) || column_name.toLowerCase() === 'id') continue;

			if (column_name.endsWith("external_id") || column_name.endsWith("stripe_id") || column_name.endsWith("legacy_id")) continue;
			...
			for (const targetTable of collections) {
				...
				if (lowerCol === `${targetPureName}_id` || lowerCol === `${targetPureName}id`) {
					isMatch = true;
					confidence += 0.2;
				} else if (targetPureName.endsWith('s') && (lowerCol === `${targetPureName.slice(0, -1)}_id` || lowerCol === `${targetPureName.slice(0, -1)}id`)) {
					isMatch = true;
					confidence += 0.2;
				}
				...
				if (isMatch) {
					...
					if (targetPk) {
						...
						const isString = (t: string) => t.includes('char') || t.includes('text');

						const typeMatch = (colType === targetPkType) ||
	...
	async hasData(collectionName: string): Promise<boolean> {
		if (!this.client) throw new Error("Not connected to PostgreSQL");
		const { schema, table } = this.parseTableSchema(collectionName);
		const result = await this.client.query(`SELECT 1 FROM "${schema}"."${table}" LIMIT 1`);
		return result.rowCount !== null && result.rowCount > 0;
	}

	async renameCollection(oldName: string, newName: string): Promise<void> {
		if (!this.client) throw new Error("Not connected to PostgreSQL");
		const { schema: oldSchema, table: oldTable } = this.parseTableSchema(oldName);
		const { table: newTable } = this.parseTableSchema(newName);

		await this.client.query(`ALTER TABLE "${oldSchema}"."${oldTable}" RENAME TO "${newTable}"`);
	}
	...
	async deleteCollection(collectionName: string): Promise<void> {
		if (!this.client) throw new Error("Not connected to PostgreSQL");
		const { schema, table } = this.parseTableSchema(collectionName);
		await this.client.query(`DROP TABLE IF EXISTS "${schema}"."${table}" CASCADE`);
	}
	 ...
	 */
	async captureSchemaSnapshot(connectionId: number, selectedSchemas?: string[]): Promise<{
	 ...
	 */
	async executeDDL(statements: Array<{ sql: string; description: string }>): Promise<{
		success: boolean;
		...
		try {
			...
			for (let i = 0; i < statements.length; i++) {
				...
				logger.log("PostgreSQLHandler", `Executing DDL (${i + 1}/${statements.length}): ${stmt.description}`);
			...
			logger.log("PostgreSQLHandler", `Successfully executed ${statements.length} DDL statements`);
