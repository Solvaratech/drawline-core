import { SchemaCollection, SchemaField, SchemaRelationship, DatabaseType } from "../types/schemaDesign";
...
export class TestDataGeneratorService {
	...
	async generateAndPopulate(
		...
		try {
			...
			await this.adapter.initialize(config, collections, relationships, config.seed);
			...
			for (const rel of filteredRelationships) {
				if (!rel.fromField) {
					warnings.push(
						`Relationship skipped: missing fromField (${rel.fromCollectionId} -> ${rel.toCollectionId})`
					);
					continue;
				}
			...
			for (const collection of generationOrder) {
				...
				try {
					...
					errors.push(
						`Schema Error ${fullName}: ${err instanceof Error ? err.message : String(err)
						}`
					);
				}
			}
			...
			for (const collection of generationOrder) {
				...
				try {
					...
					const allowedReferenceFields =
						this.getAllowedReferenceFields(
							schemaCol,
							filteredRelationships,
							collection.id
						);
					...
					const msg = `Error processing ${fullName}: ${err instanceof Error ? err.message : String(err)
						}`;
			...
			for (const collection of generationOrder) {
				...
				try {
					await this.adapter.addForeignKeyConstraints(
						fullName,
						resolvedFields
					);
				} catch (err) {
					errors.push(
						`Constraint Error ${fullName}: ${err instanceof Error ? err.message : String(err)
						}`
					);
				}
			}
	...
	private resolveForeignKeyField(
		...
		const targetName = this.collectionIdToName.get(field.referencedCollectionId);
		if (!targetName) {
			return field;
		}
