...
export abstract class BaseAdapter {
  ...
  // New
  protected insertedCompositePKRows: Map<string, Record<string, unknown>[]> =
    new Map();
   ...
   */
  abstract addForeignKeyConstraints(
    collectionName: string,
    schema: SchemaField[],
  ): Promise<void>;
  ...
  async getPrimaryKeyField(collectionName: string): Promise<string> {
    const details = await this.getCollectionDetails(collectionName).catch(
      () => null,
    );
   ...
   */
  protected generateFieldValueForFk(
    collectionName: string,
    field: SchemaField,
    index: number,
  ): string | number {
    const config =
      this.collectionConfigs.get(collectionName) ||
  ...
  protected async generateFieldValue(
    ...
    if (!isRequired) {
      const nullRate =
        field.constraints?.nullPercentage !== undefined
    ...
    switch (field.type) {
      ...
      case "integer":
      case "long":
        return this.generateInteger(field, context);
      case "number":
      case "float":
      case "decimal":
        return this.generateNumber(field, context);
      case "boolean":
        return random() > 0.5;
      case "date":
      case "timestamptz":
      case "timestamp":
        return this.generateDate(field, context);

      case "object":
      case "json":
      case "map":
        return this.generateObject(field, context);
      case "array":
      case "set":
        return this.generateArray(field, context);
      case "geopoint":
        return this.generateGeoPoint(context);
      case "binary":
      case "bytes":
        return this.generateBinary(context);

      case "uuid":
        return this.generateUUID(context.collectionName, context.documentIndex);
      case "objectid":
        return this.generateObjectIdLike(
          context.collectionName,
          context.documentIndex,
        );
      case "reference":
        return this.generateUUID(context.collectionName, context.documentIndex); // Placeholder ID (UUID for non-MongoDB)
  ...
  protected async resolveRelationships(
    ...
    const incoming =
      this.relationshipMap.byTo.get(collectionName) ||
    ...
    for (const rel of incoming) {
      ...
      if (
        ...
        (rel.type === "many-to-many" && rel.toField)
      ) {
        await this.applyRelationship(
          rel,
          collectionName,
  ...
  protected async applyRelationship(
    ...
    if (fromFields.length > 1 || toFields.length > 1) {
      await this.applyCompositeRelationship(
        rel,
        currentCollection,
        targetCollection,
        index,
    ...
    if (isIncomingOneToMany) {
      fieldName =
        rel.toField || this.inferFieldName(currentCollection, targetCollection);
    } else {
      const fromName = this.collectionIdToName.get(rel.fromCollectionId);
      if (fromName === currentCollection) {
        fieldName =
          rel.fromField ||
          this.inferFieldName(currentCollection, targetCollection);
      } else {
        fieldName =
          rel.toField ||
          this.inferFieldName(currentCollection, targetCollection);
    ...
    if (isIncomingOneToMany) {
      ...
      value = this.resolveStandardRelationship(
        rel,
        targetCollection,
        index,
        random,
  ...
  protected async applyCompositeRelationship(
    ...
    for (let i = 0; i < fromFields.length; i++) {
      ...
      const parentPkField =
        targetPkFields.find((f) => f.name === parentField) || targetPkFields[i];

      if (parentPkField) {
        const value = this.generatePKValue(
          targetCollection,
          parentPkField,
          parentIndex,
        );
  ...
  protected async resolveImplicitReferences(
    ...
    const schema =
      this.schemaMap.get(collectionName) ||
      this.schemaMap.get(collectionName.split(".").pop()!);
    if (!schema) {
      logger.warn(
        "BaseAdapter",
        `resolveImplicitReferences: No schema found for ${collectionName}`,
      );
      return;
    }
    ...
    for (const field of schema.fields) {
      ...
      if (field.compositeKeyGroup) {
        ...
        if (
          doc.data[field.name] !== undefined &&
          doc.data[field.name] !== null
        ) {
          continue;
        }
    ...
    for (const field of singleFkFields) {
      ...
      if (!targetCollectionName) {
        logger.warn(
          "BaseAdapter",
          `Could not determine target collection for FK field: ${collectionName}.${field.name}`,
        );
        continue;
      }
      ...
      if (targetCount === 0) {
        logger.warn(
          "BaseAdapter",
          `Target collection ${targetCollectionName} has 0 count for FK ${collectionName}.${field.name}`,
        );
        continue;
      }
      ...
      const docIndex =
        typeof doc.id === "number"
          ? doc.id - 1
          : parseInt(String(doc.id), 10) || 0;
      const parentIndex = Math.abs(docIndex) % targetCount;

      const targetSchema =
        this.schemaMap.get(targetCollectionName) ||
  ...
  protected async resolveCompositeFKGroup(
    ...
    if (!targetCollectionName) {
      logger.warn(
        "BaseAdapter",
        `Could not determine target for composite FK group in ${collectionName}`,
      );
      return;
    }
    ...
    if (targetCount === 0) {
      logger.warn(
        "BaseAdapter",
        `Target ${targetCollectionName} has 0 count for composite FK in ${collectionName}`,
      );
      return;
    }
    ...
    if (!targetSchema) {
      logger.warn(
        "BaseAdapter",
        `Could not find schema for target ${targetCollectionName}`,
      );
      return;
    }
    ...
    logger.log(
      "BaseAdapter",
      `Composite FK: ${collectionName} -> ${targetCollectionName}, allFields=[${allTargetFields.map((f) => f.name).join(",")}], pkFields=[${targetPkFields.map((f) => f.name).join(",")}]`,
    );

    const cachedRows =
      this.insertedCompositePKRows.get(targetCollectionName) ||
      this.insertedCompositePKRows.get(targetCollectionName.split(".").pop()!);

    if (cachedRows && cachedRows.length > 0) {
      const pickedRow = cachedRows[parentIndex % cachedRows.length];
      for (const fkField of fkFields) {
        const targetCol = fkField.foreignKeyTarget || fkField.name;
        doc.data[fkField.name] = pickedRow[targetCol];
      }
      logger.log(
        "BaseAdapter",
        `Resolved composite FK from cache for ${collectionName}, parent row: ${parentIndex}`,
      );
      return;
    }
    ...
    for (const fkField of fkFields) {
      ...
      let targetField =
        allTargetFields.find((f) => f.name === targetCol) ||
        allTargetFields.find(
          (f) => f.name.toLowerCase() === targetCol?.toLowerCase(),
        );

      if (!targetField) {
        targetField =
          targetPkFields.find((f) => f.name === targetCol) ||
          targetPkFields.find(
            (f) => f.name.toLowerCase() === targetCol?.toLowerCase(),
          );
      }
      ...
      logger.log(
        "BaseAdapter",
        `FK field ${fkField.name}: foreignKeyTarget=${fkField.foreignKeyTarget}, targetCol=${targetCol}, foundField=${targetField?.name || "NONE"} (isPK=${targetField?.isPrimaryKey})`,
      );

      if (targetField) {
        let value: string | number;
        // const value = this.generateFieldValueForFk(
        //   targetCollectionName,
        //   targetField,
        //   parentIndex,
        // );
        // logger.log(
        //   "BaseAdapter",
        //   `Generated FK value for ${fkField.name}: ${value} (targetType: ${targetField.type})`,
        // );

        // ← NEW: if the target PK column is also an FK, follow the chain
        if (
          targetField.isPrimaryKey &&
          targetField.isForeignKey &&
          targetField.referencedCollectionId
        ) {
          const grandparentCollection =
            this.resolveTargetCollection(targetField);
          if (grandparentCollection) {
            const grandparentCount = this.getCount(grandparentCollection);
            const grandparentSchema =
              this.schemaMap.get(grandparentCollection) ||
              this.schemaMap.get(grandparentCollection.split(".").pop()!);
            const grandparentPK = grandparentSchema?.fields.find(
              (f) => f.isPrimaryKey,
            );
            const grandparentIndex = parentIndex % (grandparentCount || 1);

            value = grandparentPK
              ? this.generatePKValue(
                  grandparentCollection,
                  grandparentPK,
                  grandparentIndex,
                )
              : this.generateId(grandparentCollection, grandparentIndex);
          } else {
            value = this.generateFieldValueForFk(
              targetCollectionName,
              targetField,
              parentIndex,
            );
          ...
          // original path — target PK is not an FK
          value = this.generateFieldValueForFk(
            targetCollectionName,
            targetField,
            parentIndex,
        ...
        logger.warn(
          "BaseAdapter",
          `No target field found for ${fkField.name}, using fallback (fieldType=${fieldType}, rawType=${rawType})`,
        );

        if (fieldType === "uuid" || rawType.includes("uuid")) {
          doc.data[fkField.name] = this.generateUUID(
            targetCollectionName,
            parentIndex,
          );
        } else if (
          ...
          doc.data[fkField.name] = this.generateId(
            targetCollectionName,
            parentIndex,
          );
        }
    ...
    logger.log(
      "BaseAdapter",
      `Resolved composite FK group for ${collectionName}, parent row: ${parentIndex}`,
    );
  }
  ...
  protected resolveTargetCollection(field: SchemaField): string | undefined {
    ...
    const resolvedName = this.collectionIdToName.get(
  ...
  generateId(collectionName: string, index: number): string | number {
    ...
    if (idType === "integer" || idType === "long" || idType === "bigint")
      return effectiveIndex + 1;
    if (idType === "uuid")
      return this.generateUUID(collectionName, effectiveIndex);
    ...
    throw new Error(
      `Unsupported ID type: '${idType}' for collection '${collectionName}'.`,
    );
  }
  ...
  protected async preloadMetadata(
    collections: SchemaCollection[],
    config: TestDataConfig,
  ): Promise<void> {
    const configMap = new Map(
      config.collections.map((c) => [c.collectionName, c.count]),
    );

    for (const col of collections) {
      const count =
        configMap.get(col.name) ?? // "users"
        configMap.get(`${col.schema}.${col.name}`) ?? // "public.users"  ← added this to map schema.name
        configMap.get((col as any).dbName) ?? // dbName fallback
        0;

      const fullDbName =
        (col as any).dbName ||
        (col.schema && col.schema !== "public"
          ? `${col.schema}.${col.name}`
          : col.name);
      ...
      const pkField =
        col.fields.find((f) => f.isPrimaryKey) ||
        col.fields.find((f) => f.name === "id");
      ...
      if (pkField) {
        ...
        if (
          details.primaryKeyType === "integer" ||
          details.primaryKeyType === "number" ||
          details.primaryKeyType === "long"
        )
          idType = "integer";
      ...
      try {
        ...
        if (dbSchema && dbSchema.length > 0) {
          logger.log(
            "BaseAdapter",
            `Syncing schema for ${col.name} with ${dbSchema.length} DB fields`,
          );
          for (const dbField of dbSchema) {
            const schemaField = col.fields.find((f) => f.name === dbField.name);
            if (schemaField) {
              if (
                dbField.constraints?.enum &&
                dbField.constraints.enum.length > 0
              ) {
                logger.log(
                  "BaseAdapter",
                  `Syncing Enum for ${col.name}.${dbField.name}:`,
                  dbField.constraints.enum,
                );
              ...
              if (dbField.constraints?.unique) {
                logger.log(
                  "BaseAdapter",
                  `Enforcing unique for ${col.name}.${dbField.name}`,
                );
                schemaField.constraints = {
                  ...schemaField.constraints,
                  unique: true,
              ...
              if (
                ...
                (dbC.min !== undefined ||
                  dbC.max !== undefined ||
                  dbC.minLength !== undefined ||
                  dbC.maxLength !== undefined ||
                  dbC.minColumn ||
                  dbC.maxColumn ||
                  dbC.gtColumn ||
                  dbC.ltColumn)
              ...
              if (dbField.type && schemaField.type !== dbField.type) {
                ...
                if (dbField.type === "uuid") {
                  ...
                  (dbField.type === "integer" || dbField.type === "long") &&
                  (oldType === "string" ||
                    oldType === "reference" ||
                    oldType === "objectid")
                ) {
                  ...
                  (dbField.type === "date" ||
                    dbField.type === "timestamp" ||
                    dbField.type === "timestamptz") &&
                  oldType === "string"
                ) {
                  schemaField.type = dbField.type;
                } else if (
                  dbField.type === "boolean" &&
                  oldType !== "boolean"
                ) {
              ...
              if (dbField.compositePrimaryKeyIndex !== undefined) {
                schemaField.compositePrimaryKeyIndex =
                  dbField.compositePrimaryKeyIndex;
              }
              if (dbField.isForeignKey) {
                schemaField.isForeignKey = true;
                if (dbField.referencedCollectionId) {
                  schemaField.referencedCollectionId =
                    dbField.referencedCollectionId;
                }
  ...
  protected buildRelationshipMap(relationships: SchemaRelationship[]): void {
    ...
    for (const rel of relationships) {
      ...
      const fromName = this.collectionIdToName.get(rel.fromCollectionId);
      if (fromName) {
        if (!this.relationshipMap.byFrom.has(fromName))
          this.relationshipMap.byFrom.set(fromName, []);
        this.relationshipMap.byFrom.get(fromName)!.push(rel);
      }

      const toName = this.collectionIdToName.get(rel.toCollectionId);
      if (toName) {
        if (!this.relationshipMap.byTo.has(toName))
          this.relationshipMap.byTo.set(toName, []);
  ...
  protected generateGeoPoint(context: FieldGenerationContext): {
    lat: number;
    lng: number;
  } {
  ...
  protected generateInteger(
    ...
    if (field.constraints?.unique) {
      const start =
        (this.collectionConfigs.get(context.collectionName)?.startId ?? 0) + 1;
  ...
  protected generateDate(
    ...
    generatedDate = this.applyCrossColumnConstraints(
      generatedDate,
      field,
      context,
    ) as Date;
      ...
      if (minVal !== undefined) {
        if (adjusted < minVal) {
          if (adjusted instanceof Date) {
            ...
            const offset =
              60 * 1000 +
              Math.floor(context.random() * 10 * 24 * 60 * 60 * 1000);
      ...
      if (gtVal !== undefined) {
        if (adjusted <= gtVal) {
          if (adjusted instanceof Date) {
            ...
            adjusted =
              (gtVal as number) + Math.max(1, Math.abs(adjusted as number));
          }
        }
      }
    }
  ...
  protected inferFieldName(
    ...
    const candidates = [`${lowerTarget}Id`, `${lowerTarget}_id`, lowerTarget];

    for (const field of collection.fields) {
      if (candidates.includes(field.name)) return field.name;
      if (
        field.isForeignKey &&
        field.referencedCollectionId === targetCollection
      )
        return field.name;
    }
