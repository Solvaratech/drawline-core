...
export class PostgresAdapter extends BaseAdapter {
	...
	private schemaCache: Map<string, { columns: Map<string, string>; autoIncrement: Set<string> }> = new Map();
	...
	async insertDocuments(
		...
		const hasExplicitId = documents.some((d) => d.id !== undefined && d.id !== null);
		if (
			hasExplicitId &&
			primaryKey &&
		...
		logger.log("PostgresAdapter", `Inserting ${documents.length} rows into ${collectionName}`);
		...
		for (let i = 0; i < documents.length; i += batchSize) {
			...
			try {
				if (collectionName.includes('order_items')) {
					logger.log("PostgresAdapter", `Inserting ${collectionName}: columns=${columns.join(',')}, values0=${JSON.stringify(values.slice(0, columns.length))}`);
				}
		...
		if (insertedIds.length === 0 && documents.length > 0) {
			return documents.map((d) => d.id).filter((id): id is string | number => id !== undefined);
		}
	...
	async getDocumentCount(collectionName: string): Promise<number> {
		if (!this.client) throw new Error("Not connected to PostgreSQL");
		const { schema, table } = this.parseTableSchema(collectionName);
		const result = await this.client.query(`SELECT COUNT(*) as count FROM "${schema}"."${table}"`);
	...
	async ensureCollection(collectionName: string, schema?: SchemaField[], skipForeignKeys: boolean = false): Promise<void> {
		if (!this.client) throw new Error("Not connected to PostgreSQL");

		const exists = await this.collectionExists(collectionName);
		if (exists) {
			logger.log("PostgresAdapter", `Table ${collectionName} already exists, skipping creation`);
			return;
		}
		...
		const columns = schema.map((field) => {
			...
			if (field.isPrimaryKey) {
				...
				if (!skipForeignKeys && field.isForeignKey && field.referencedCollectionId && !field.compositeKeyGroup) {
					const targetCol = field.foreignKeyTarget || "id";
					const { schema: targetSchema, table: targetTable } = this.parseTableSchema(field.referencedCollectionId);
					const targetTableQuoted = `"${targetSchema}"."${targetTable}"`;

					logger.log("PostgresAdapter", `Adding FK to ${collectionName}.${field.name} referencing ${targetTableQuoted}.${targetCol}`);
		...
		if (!skipForeignKeys) {
			compositeFKGroups.forEach((fields, groupName) => {
				...
				const remoteCols = fields.map(f => `"${f.foreignKeyTarget || 'id'}"`).join(", ");

				const { schema: targetSchema, table: targetTable } = this.parseTableSchema(refTable);
				const targetTableQuoted = `"${targetSchema}"."${targetTable}"`;

				logger.log("PostgresAdapter", `Adding composite FK ${groupName} to ${collectionName} referencing ${targetTableQuoted}`);
				tableConstraints.push(`FOREIGN KEY (${localCols}) REFERENCES ${targetTableQuoted} (${remoteCols})`);
			});
		}
	...
	async addForeignKeyConstraints(collectionName: string, schema: SchemaField[]): Promise<void> {
		...
		const fkFields = schema.filter(f => f.isForeignKey && f.referencedCollectionId);
		...
		for (const [groupName, fields] of compositeFKGroups) {
			...
			try {
				await this.client.query(query);
				logger.log("PostgresAdapter", `Added composite FK constraint ${constraintName}`);
			} catch (error: any) {
				if (error.code === '42710') { // duplicate_object
					// ignore
				} else {
					logger.warn("PostgresAdapter", `Failed to add composite FK constraint ${constraintName}:`, error.message);
		...
		for (const field of singleFKFields) {
			...
			try {
				...
				if (error.code === '42710') { // duplicate_object
					...
					logger.warn("PostgresAdapter", `Failed to add FK constraint ${constraintName}:`, error.message);
					if (error.code === '23503') throw error;
				}
			}
		}
	...
	async updateSequence(collectionName: string): Promise<void> {
		...
		try {
			...
			logger.warn("PostgresAdapter", `Failed to update sequence for ${collectionName}`, e);
		}
	}
	...
	async getCollectionDetails(collectionName: string): Promise<CollectionDetails> {
		...
		if (result.rows.length === 0) {
			const details: CollectionDetails = { primaryKey: "id", primaryKeyType: "string", isCompositePK: false };
		...
		if (firstIntPKIdx >= 0) {
					...
					try {
						...
						if (seqName) {
							...
							if (lastValRes.rows.length > 0) {
								...
								logger.log("PostgresAdapter", `${table}.${pkCol} sequence at ${lastValue} (is_called=${isCalled}), startId=${startId}`);
							}
						}
					} catch (seqErr) {
						logger.warn("PostgresAdapter", `No sequence found for ${table}.${pkCol}`);
					}
				}
			} catch {
				logger.warn("PostgresAdapter", `Failed to get MAX ID for ${collectionName}`);
			}
		} else {
			...
			try {
				...
				logger.warn("PostgresAdapter", `Failed to get COUNT for ${collectionName}`);
			}
		}
	...
	private parseTableSchema(collectionName: string): { schema: string; table: string } {
	...
	private async resolveCollectionName(collectionName: string): Promise<{ schema: string; table: string }> {
		if (collectionName.includes(".")) {
		...
		try {
			...
			if (res.rows.length === 1) {
				...
				if (!hasPublic) {
					logger.warn("PostgresAdapter", `Ambiguous table '${collectionName}' found in multiple schemas: ${res.rows.map(r => r.table_schema).join(', ')}. Defaulting to public.`);
				}
			}
		} catch (e) {
	...
	private parseCheckConstraints(
		columnName: string,
		checkDefs: string[]
	): { min?: number; max?: number; positive?: boolean; minLength?: number; maxLength?: number; minColumn?: string; maxColumn?: string; gtColumn?: string; ltColumn?: string; enum?: string[] } {
		const result: { min?: number; max?: number; positive?: boolean; minLength?: number; maxLength?: number; minColumn?: string; maxColumn?: string; gtColumn?: string; ltColumn?: string; enum?: string[] } = {};
		...
		for (const def of checkDefs) {
			...
			if (lengthBetweenMatch && lengthBetweenMatch[1] === colPattern) {
				...
				logger.log("PostgresAdapter", `Parsed length constraint for ${columnName}: ${result.minLength}-${result.maxLength}`);
			}

			// Pattern: length(column) >= N
			const lengthGteMatch = normalized.match(/length\s*\(\s*(\w+)\s*\)\s*>=\s*(\d+)/);
			if (lengthGteMatch && lengthGteMatch[1] === colPattern) {
				result.minLength = parseInt(lengthGteMatch[2], 10);
			}

			// Pattern: length(column) > N
			const lengthGtMatch = normalized.match(/length\s*\(\s*(\w+)\s*\)\s*>\s*(\d+)/);
			if (lengthGtMatch && lengthGtMatch[1] === colPattern) {
				result.minLength = parseInt(lengthGtMatch[2], 10) + 1;
			}

			// Pattern: length(column) <= N
			const lengthLteMatch = normalized.match(/length\s*\(\s*(\w+)\s*\)\s*<=\s*(\d+)/);
			if (lengthLteMatch && lengthLteMatch[1] === colPattern) {
				result.maxLength = parseInt(lengthLteMatch[2], 10);
			}

			// Pattern: length(column) < N
			const lengthLtMatch = normalized.match(/length\s*\(\s*(\w+)\s*\)\s*<\s*(\d+)/);
			if (lengthLtMatch && lengthLtMatch[1] === colPattern) {
				result.maxLength = parseInt(lengthLtMatch[2], 10) - 1;
			}

			// Pattern: column > N
			const gtMatch = normalized.match(new RegExp(`${colPattern}\\s*>\\s*(-?\\d+(?:\\.\\d+)?)`));
			if (gtMatch) {
			...
			// Pattern: column >= N
			const gteMatch = normalized.match(new RegExp(`${colPattern}\\s*>=\\s*(-?\\d+(?:\\.\\d+)?)`));
			if (gteMatch) {
			...
			// Pattern: column < N
			const ltMatch = normalized.match(new RegExp(`${colPattern}\\s*<\\s*(-?\\d+(?:\\.\\d+)?)`));
			if (ltMatch) {
			...
			// Pattern: column <= N
			const lteMatch = normalized.match(new RegExp(`${colPattern}\\s*<=\\s*(-?\\d+(?:\\.\\d+)?)`));
			if (lteMatch) {
			...
			const betweenMatch = normalized.match(
				new RegExp(`${colPattern}\\s+between\\s+(-?\\d+(?:\\.\\d+)?)\\s+and\\s+(-?\\d+(?:\\.\\d+)?)`)
			);
			if (betweenMatch) {
			...
			const crossColGteMatch = normalized.match(new RegExp(`${colPattern}\\s*>=\\s*(\\w+)`));
			if (crossColGteMatch) {
				const otherCol = crossColGteMatch[1];
				if (!/^-?\d+(\.\d+)?$/.test(otherCol)) {
					result.minColumn = otherCol;
					logger.log("PostgresAdapter", `Parsed cross-column constraint: ${columnName} >= ${otherCol}`);
				}
			}

			// Pattern: column > other_column
			const crossColGtMatch = normalized.match(new RegExp(`${colPattern}\\s*>\\s*(\\w+)`));
			...
			// Pattern: column <= other_column
			const crossColLteMatch = normalized.match(new RegExp(`${colPattern}\\s*<=\\s*(\\w+)`));
			if (crossColLteMatch) {
			...
			// Pattern: column < other_column
			const crossColLtMatch = normalized.match(new RegExp(`${colPattern}\\s*<\\s*(\\w+)`));
			...
			const inMatch = normalized.match(new RegExp(`${colPattern}.*?\\s+in\\s+\\(([^)]+)\\)`));
			if (inMatch) {
				...
				if (values && values.length > 0) {
					result.enum = values;
					logger.log("PostgresAdapter", `Parsed IN constraint for ${columnName}: ${values.join(', ')}`);
				}
			}
			...
			if (!result.enum) {
				...
				if (arrayMatch) {
					...
					if (values && values.length > 0) {
						result.enum = values;
						logger.log("PostgresAdapter", `Parsed ANY(ARRAY) constraint for ${columnName}: ${values.join(', ')}`);
					}
				}
			}
		}
	...
	`;
		...
		for (const row of result.rows) {
			...
			if (pgType.startsWith("bigint") || pgType.startsWith("integer") || pgType.startsWith("smallint")) type = "integer";
			else if (pgType.startsWith("numeric") || pgType.startsWith("decimal")) type = "decimal";
			else if (pgType.startsWith("double precision") || pgType.startsWith("real")) type = "float";
			else if (pgType.startsWith("boolean")) type = "boolean";
			else if (pgType.startsWith("timestamp with time zone")) type = "timestamptz";
			...
			if (row.enum_values) {
				if (Array.isArray(row.enum_values)) {
					...
					enumValues = raw.substring(1, raw.length - 1).split(',').map(v => {
			...
			const checkBounds = this.parseCheckConstraints(row.column_name, checkDefs);

			if (checkBounds.min !== undefined || checkBounds.max !== undefined ||
				checkBounds.minLength !== undefined || checkBounds.maxLength !== undefined) {
				logger.log("PostgresAdapter", `CHECK constraint for ${table}.${row.column_name}: min=${checkBounds.min}, max=${checkBounds.max}, minLength=${checkBounds.minLength}, maxLength=${checkBounds.maxLength}`);
			}

			const fkColumnCount = row.fk_column_count ? parseInt(row.fk_column_count, 10) : 1;
			const compositeKeyGroup = isForeignKey && fkColumnCount > 1 ? row.fk_constraint_name : undefined;

			const pkColumnCount = row.pk_column_count ? parseInt(row.pk_column_count, 10) : 1;
			const compositePrimaryKeyIndex = isPrimaryKey && pkColumnCount > 1 && row.pk_position
			...
			fields.push({
				...
				referencedCollectionId: isForeignKey ? (row.foreign_schema === 'public' ? row.foreign_table : `${row.foreign_schema}.${row.foreign_table}`) : undefined,
	...
	async buildDependencyOrder(
		...
		for (const rel of relationships) {
			...
			if (rel.fromFields) {
				for (const f of rel.fromFields) explicitRelFields.add(`${fromName}|${f}`);
			}
			...
			if (rel.fromField) {
				...
				if (fkField && fkField.required !== true && fkField.nullable !== false) {
		...
		for (const col of collections) {
			for (const field of col.fields) {
				...
				const targetName = collectionIdToName.get(field.referencedCollectionId) || field.referencedCollectionId;
		...
		while (processed.size < collections.length) {
			if (queue.length === 0) {
				...
				logger.warn("PostgresAdapter", `Cycle detected, breaking at: ${candidate.name}`);
		...
		logger.log("PostgresAdapter", `Dependency order: ${result.map(c => c.name).join(' -> ')}`);
		return result;
	}
}